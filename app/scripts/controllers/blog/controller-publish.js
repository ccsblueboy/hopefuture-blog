    ...
    publishService.edit(id, function (data) {
      if (data.success === true) {
        ...
        for (i = 0, len = labels.length; i < len; i++) {
          _labels.push(labels[i].name);
          $scope.labelCollection.add({name: labels[i].name});
        }
        $scope.article.labels = _labels;

        var categories = $scope.article.categories;
        var _categories = [];
        for (i = 0, len = categories.length; i < len; i++) {
          _categories.push(categories[i]._id);
        }
        $scope.article.categories = _categories;
      }
    });
      ...
      publishService.getLabel(function (data) {
        if (data.success === true) {
          ...
          if (data.items.length > 0) {
            var minCount = data.items[0].count, maxCount = data.items[data.items.length - 1].count, sub = maxCount - minCount;
            /**
